...
const Profile: React.FC<Props> = ({ setView, currentCurrency, onCurrencyChange, terms, user, currencies, rateAppLink, onLogout, onUpdateUser }) => {
  ...
  const handleCopyID = (e: React.MouseEvent) => {
      e.stopPropagation(); // Stop event bubbling so it doesn't trigger parent clicks
      if (user?.id) {
          navigator.clipboard.writeText(user.id);
          setIdCopied(true);
          setTimeout(() => setIdCopied(false), 2000); // Reset after 2 seconds
      }
  };
  ...
  return (
    <div className="min-h-screen pb-24 bg-[#13141f] relative pt-6">
      ...
      {/* User Info Card (Redesigned as div container) */}
      <div className="w-full px-4 mb-8 flex items-center gap-4 text-right">
         
         {/* Avatar Section - Click to Edit */}
         ...
         <div className="flex-1 flex flex-col items-end">
            <div className="flex items-center gap-2 cursor-pointer" onClick={handleOpenEdit}>
                <h2 className="text-white font-bold text-xl hover:text-yellow-400 transition-colors">{user?.name || 'زائر'}</h2>
            </div>
            
            {/* ID Section - Independent Clickable Element for Copying */}
            <button 
                onClick={handleCopyID}
                className={`flex items-center gap-1.5 mt-1 px-2 py-1 rounded-md border transition-all active:scale-95 ${idCopied ? 'bg-green-500/10 border-green-500/50' : 'bg-[#242636] border-gray-800 hover:border-yellow-400/50'}`}
                title="نسخ المعرف"
            >
                {idCopied ? <Check size={12} className="text-green-500" /> : <Copy size={10} className="text-gray-600" />}
                <p className={`text-xs font-bold dir-ltr font-mono transition-colors ${idCopied ? 'text-green-500' : 'text-gray-500'}`}>
                    ID: {user?.id || '---'}
                </p>
                {idCopied && <span className="text-[9px] text-green-500 font-bold mr-1">تم النسخ</span>}
            </button>
         </div>

         {/* Chevron - Click to Edit */}
         <button onClick={handleOpenEdit} className="text-gray-600 hover:text-yellow-400 transition-colors p-2">
