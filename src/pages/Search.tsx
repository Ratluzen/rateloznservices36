...
const SearchPage: React.FC<Props> = ({ setView, formatPrice, products, categories, addToCart, userBalance = 0, onPurchase, onProductSelect }) => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (selectedPrice === 'under_10') matchesPrice = product.price < 10;
    else if (selectedPrice === '10_50') matchesPrice = product.price >= 10 && product.price <= 50;
    else if (selectedPrice === 'over_50') matchesPrice = product.price > 50;
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-colors border flex items-center gap-2 ${selectedCategory === cat.id ? 'bg-yellow-400 text-black border-yellow-400' : 'bg-[#242636] text-gray-300 border-gray-700 hover:border-gray-500'}`}
              >
                <cat.icon size={14} />
                {cat.name}
              </button>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {priceOptions.map((option) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-colors border ${selectedPrice === option.id ? 'bg-yellow-400 text-black border-yellow-400' : 'bg-[#242636] text-gray-300 border-gray-700 hover:border-gray-500'}`}
              >
                {option.label}
              </button>
            ))}
      ...
      {filteredProducts.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center mt-12 px-10 text-center animate-fadeIn">
            <div className="w-32 h-32 bg-[#FCD34D] rounded-full flex items-center justify-center mb-6 relative overflow-hidden shadow-lg">
                ...
                <div className="w-9 h-9 bg-white rounded-full flex items-center justify-center relative overflow-hidden"><div className="w-4 h-4 bg-[#1f2937] rounded-full absolute right-1"></div></div>
                <div className="w-9 h-9 bg-white rounded-full flex items-center justify-center relative overflow-hidden"><div className="w-4 h-4 bg-[#1f2937] rounded-full absolute right-1"></div></div>
                </div>
            </div>
